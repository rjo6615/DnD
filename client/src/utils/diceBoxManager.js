...
const ensureElementSelector = (element) => {
  ...
  if (existingId) {
    ...
    try {
      ...
      // Fall through and assign a generated selector if the existing id cannot
      // be queried safely.
    }
  }
  ...
  const generatedSelector = `[data-dice-box-host="${generatedId}"]`;

  try {
    element.setAttribute('data-dice-box-host', generatedId);
  } catch {
    return null;
  }
