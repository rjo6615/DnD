...
test('cast button disabled until spell checked and then calls onCastSpell', async () => {
  ...
  await screen.findByLabelText('Level');
  await userEvent.selectOptions(screen.getByLabelText('Level'), '3');
  const fireballLabel = await screen.findByText('Fireball');
  const fireballCard = fireballLabel.closest('.spell-card');
  expect(fireballCard).not.toBeNull();
  const castBtn = within(fireballCard).getAllByRole('button')[1];
  expect(castBtn).toBeDisabled();
  const checkbox = within(fireballCard).getByRole('checkbox');
...
test.each([
  ...
  const fireBoltLabel = await screen.findByText('Fire Bolt');
  const fireBoltCard = fireBoltLabel.closest('.spell-card');
  expect(fireBoltCard).not.toBeNull();
  await userEvent.click(within(fireBoltCard).getByRole('checkbox'));
  const castBtn = within(fireBoltCard).getAllByRole('button')[1];
  await userEvent.click(castBtn);
  expect(onCast).toHaveBeenCalledWith(
...
test('UpcastModal returns warlock slot type', async () => {
  ...
  await screen.findByLabelText('Level');
  await userEvent.selectOptions(screen.getByLabelText('Level'), '1');
  const spellLabel = await screen.findByText('Burning Hands');
  const spellCard = spellLabel.closest('.spell-card');
  expect(spellCard).not.toBeNull();
  await userEvent.click(within(spellCard).getByRole('checkbox'));
  const castBtn = within(spellCard).getAllByRole('button')[1];
  await userEvent.click(castBtn);
...
test('upcasting consumes higher slot and reports extra damage', async () => {
  ...
  await screen.findByLabelText('Level');
  await userEvent.selectOptions(screen.getByLabelText('Level'), '1');
  const burningHandsLabel = await screen.findByText('Burning Hands');
  const burningHandsCard = burningHandsLabel.closest('.spell-card');
  expect(burningHandsCard).not.toBeNull();
  await userEvent.click(within(burningHandsCard).getByRole('checkbox'));
  const castBtn = within(burningHandsCard).getAllByRole('button')[1];
  await userEvent.click(castBtn);
